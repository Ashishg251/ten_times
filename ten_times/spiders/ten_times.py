...
class TenTimesSpider(Spider):
    ...
    start_urls = [
        'https://10times.com/australia/tradeshows?datefrom=2021-09-25&dateto=2021-12-30']
    ...
    def parse(self, response):
        ...
        self.driver = webdriver.Chrome(
            '/Users/mmt8856/Downloads/chromedriver', desired_capabilities=caps)
        self.driver.get(
            'https://10times.com/australia/tradeshows?datefrom=2021-09-25&dateto=2021-12-30')
        ...
        while True:
            ...
            time.sleep(2.2)
        try:
            modal_window = self.driver.find_element_by_xpath(
                '//*[@id="_modal_close"]')
            modal_window.click()
        ...
        events_selector = scrapy_selector.xpath(
            '//*[@aria-label="evt_bkmrk"]')  # returns the list of selectors
        self.logger.info('Theres a total of ' +
                         str(len(events_selector)) + ' links.')
        event_urls_distinct = []
        ...
        # Opening JSON file
        # file_path = '/Users/ashish/Documents/a1-shows/ten_times/data/url.json'
        # f = open(file_path)
        ...
        q = 0
        self.logger.info("event_urls_distinct length = " +
                         str(len(event_urls_distinct)))
        for event_url in event_urls_distinct:
            try:
                ...
                self.logger.info(
                    "sleep time for events = " + str(q/sleep_time))
                link_to_home = event_url
                event_scrapy_selector = Selector(text=self.driver.page_source)
                event_json_string = event_scrapy_selector.xpath(
                    '//*[@type="application/ld+json"]/text()').extract_first()
                event_schema_json = json.loads(event_json_string)

                # scraping event metadata started
                visitors = event_scrapy_selector.css('table.table.noBorder.mng tr')[
                    1].css('td a[href*=visitors]::text').get()
                if visitors is not None and hasNumbers(visitors):
                    exhibitors = event_scrapy_selector.css('table.table.noBorder.mng tr')[
                        1].css('td a[href*=exhibitors]::text').get()
                ...
                event_schema_json["organizer_name"] = event_scrapy_selector.xpath(
                    '//*[@id="org-name"]/text()').extract_first()
                self.logger.info("Organizer Name = " +
                                 event_schema_json["organizer_name"])
                event_schema_json["organizer_link"] = event_scrapy_selector.xpath(
                    '//*[@id="org-name"]/@href').extract_first()
                if event_schema_json["organizer_link"] not in organizer_links_distinct and event_schema_json["organizer_link"] is not None:
                    organizer_links_distinct.append(
                        event_schema_json["organizer_link"])
        ...
        self.logger.info("organizer link length = " +
                         str(len(organizer_links_distinct)))
        j = 0
        for organizer_link in organizer_links_distinct:
            try:
                ...
                organizer_scrapy_selector = Selector(
                    text=self.driver.page_source)
                organizer_json_string = organizer_scrapy_selector.xpath(
                    '//*[@type="application/ld+json"]/text()').extract_first()
                ...
                self.logger.info(
                    "sleep time for organizers = " + str(j/sleep_time))
                j = j+1
            except Exception as e:
                self.logger.info("exception is = " + str(e))
                continue
